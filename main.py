...
def menu(uid):
    ...
    txt = (
        ...
        f"💎 <b>Balance:</b> <code>{coins} Coins</code>\n"
        "────────────────────\n"
        f"🎁 <b>Per Join:</b> +{jr} Coins\n"
        f"👥 <b>Per Refer:</b> +{rr} Coins\n"
        f"🚀 <b>Promo Cost:</b> {cp} Coins/Sub\n"
        "────────────────────"
    )
...
def add_camp(call):
    ...
    if get_coins(uid) < cost:
        ...
        return
    msg = bot.send_message(call.message.chat.id, f"1. Bot <b>@{BOT_USERNAME}</b> (ID: <code>{BOT_ID}</code>) ko channel me ADMIN banao.\n2. Username bhejo (@MyChannel):")
    bot.register_next_step_handler(msg, step_verify_admin)

def step_verify_admin(m):
    ch_u = m.text.strip()
    if not ch_u.startswith("@"):
        bot.send_message(m.chat.id, "❌ '@' se start karein.")
        return
    try:
        b = bot.get_chat_member(ch_u, BOT_ID)
        if b.status not in ["administrator", "creator"]:
            bot.send_message(m.chat.id, f"❌ Bot @{BOT_USERNAME} channel me Admin nahi hai!")
            return
    except Exception:
        bot.send_message(m.chat.id, "❌ Channel read error. Public channel check karein.")
        return
    cost = get_setting('cost_per_member', 15)
...
def step_save_camp(m, ch_u):
    try:
        ...
        if cnt <= 0:
            bot.send_message(m.chat.id, "❌ Kam se kam 1 member dalein.")
            return
        cost = get_setting('cost_per_member', 15)
        tot = cnt * cost
        uid = m.from_user.id
...
def stats_show(call):
    ...
    r = c.fetchone()[0]
    txt = f"📊 <b>DASHBOARD</b>\n────────────────────\n💎 Balance: <code>{coins} Coins</code>\n👥 Referrals: <code>{r}</code>\n✅ Joined: <code>{j}</code>"
...
def adm_panel(call):
    ...
    cost = get_setting("cost_per_member", 15)
    txt = f"⚡ <b>ADMIN CONSOLE</b> ⚡\n────────────────────\nUsers: <code>{u_c}</code> | Campaigns: <code>{c_c}</code>\n• Join: {j} | Refer: {r} | Cost: {cost}"
    kb = types.InlineKeyboardMarkup(row_width=2)
    kb.add(
